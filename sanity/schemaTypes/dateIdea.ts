...
export const dateIdeaType = defineType({
  name: 'date_ideas',
  title: 'Date ideas',
  type: 'document',
  fields: [
    defineField({
      name: 'name',
      type: 'string',
      description: 'Title of the date idea.',
      validation: (rule) => rule.required(),
    }),
    defineField({
      name: 'slug',
      type: 'slug',
      description: 'URL-friendly identifier for the date idea.',
      options: {source: 'name'},
      validation: (rule) => rule.required(),
    }),
    defineField({
      ...
      type: 'boolean',
      description: 'Indicates if the date idea has a specific date and time.',
      initialValue: false,
    }),
    defineField({
      name: 'start_datetime',
      type: 'datetime',
      hidden: ({document}) => !document?.has_date_and_time || Boolean(document?.all_day),
      validation: (rule) =>
        rule.custom((value, context) => {
          if (!context.document?.has_date_and_time) return true;
    ...
    defineField({
      ...
      hidden: ({document}) => !document?.has_date_and_time || !document?.all_day,
      validation: (rule) =>
        rule.custom((value, context) => {
          if (!context.document?.has_date_and_time) return true;
          if (!context.document?.all_day) return true;
    ...
    defineField({
      ...
      initialValue: false,
      hidden: ({document}) => !document?.has_date_and_time,
    }),
    defineField({
      name: 'recurring',
      type: 'boolean',
    ...
    defineField({
      ...
      hidden: ({document}) =>
        !document?.has_date_and_time ||
        !document?.recurring ||
        document?.recurrence_frequency !== 'weekly',
      validation: (rule) =>
        rule.custom((value, context) => {
          if (!context.document?.has_date_and_time) return true;
          if (!context.document?.recurring) return true;
